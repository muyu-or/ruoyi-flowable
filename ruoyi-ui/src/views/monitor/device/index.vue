    ...
    <el-form v-show="showSearch" ref="queryForm" :model="queryParams" size="small" :inline="true" label-width="80px">
      ...
      <el-form-item label="区域类型" prop="areaType">
        <el-select v-model="queryParams.areaType" placeholder="请选择区域类型" clearable>
          <el-option
            v-for="dict in dict.type.monitor_area_type"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
      ...
      <el-form-item label="设备状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择设备状态" clearable>
          <el-option
            v-for="dict in dict.type.monitor_device_status"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="deviceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="区域类型" align="center" prop="areaType" width="100">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.monitor_area_type" :value="scope.row.areaType" />
        </template>
      </el-table-column>
      <el-table-column label="设备类型" align="center" prop="deviceType" width="100">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.monitor_device_type" :value="scope.row.deviceType" />
        </template>
      </el-table-column>
      <el-table-column label="品牌型号" align="center" prop="brand" :show-overflow-tooltip="true" />
      <el-table-column label="设备状态" align="center" prop="status" width="80">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.monitor_device_status" :value="scope.row.status" />
        </template>
      </el-table-column>
          ...
          <el-col :span="12">
            <el-form-item label="设备类型" prop="deviceType">
              <el-select v-model="form.deviceType" placeholder="请选择设备类型" style="width: 100%">
                <el-option
                  v-for="dict in dict.type.monitor_device_type"
                  :key="dict.value"
                  :label="dict.label"
                  :value="dict.value"
                />
...
<script>
import { listDevice, getDevice, delDevice, addDevice, updateDevice } from '@/api/monitor/device'

export default {
  name: 'MonitorDevice',
  dicts: ['monitor_area_type', 'monitor_device_type', 'monitor_device_status'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除监控设备编号为"' + ids + '"的数据项？').then(function() {
