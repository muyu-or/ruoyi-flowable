...
<script>
import { listTemplate, getTemplate, delTemplate, addTemplate, updateTemplate, previewTemplate } from '@/api/manage/template'
...
export default {
  ...
  methods: {
    ...
    handleStatusChange(row) {
      const text = row.reportStatus === 1 ? '启用' : '停用'
    ...
    handlePreview(row) {
      ...
      // xlsx/xls：调用后端接口，返回含 Base64 图片的 HTML
      if (fileType === 'xlsx' || fileType === 'xls') {
        previewTemplate(row.storagePath).then(res => {
